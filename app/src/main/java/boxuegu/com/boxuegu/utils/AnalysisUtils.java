...
public class AnalysisUtils {
    ...
    static public List<ExercisesDetailBean> getExercisesInfos(InputStream inputStream) throws Exception {
        //该方法是解析inputStream所对应的文件，返回解析的选择题集合，p153
        XmlPullParser parser = Xml.newPullParser();//获取/生成一个解析器对象
        List<ExercisesDetailBean> beans = null;
        ExercisesDetailBean bean = null;
        parser.setInput(inputStream, "utf-8");
        int type = parser.getEventType();
        while (type != XmlPullParser.END_DOCUMENT) {
            //下面分析文件里面的内容，为这个元素的属性赋值
            switch (type) {
                ...
                case XmlPullParser.END_TAG: {
                    if ("exercises".equals(parser.getName())) {
                        bean.selected = 0;//现在用户还没有选择一个答案，如果用户选择了A答案，则赋值为1
                        beans.add(bean);
                        bean = null;
                    }
                    break;
